...
static inline int
pct_to_pixels(int total, unsigned int pct)
{
	// do signed arithmetic to avoid overflow. sanity check & clamp pixels
	// first.
	if (total > INT_MAX / 100)
		return (total / 100) * (int)pct;
...
static void
draw_discharging(unsigned int left)
{
...
static void
draw_charging(unsigned int left)
{
